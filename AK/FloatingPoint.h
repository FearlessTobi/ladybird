...
using F80ComponentType = unsigned long long;
...
#    if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
    F80ComponentType sign : 1;
    F80ComponentType exponent : 15;
    F80ComponentType mantissa : 64;
#    else
    F80ComponentType mantissa : 64;
    F80ComponentType exponent : 15;
    F80ComponentType sign : 1;
...
union FloatExtractor<f80> {
    ...
    static constexpr F80ComponentType mantissa_max = ~0ull;
    static constexpr int exponent_bias = 16383;
    static constexpr int exponent_bits = 15;
    static constexpr unsigned exponent_max = 32767;
    [[gnu::packed]] F80Fields fields;
    f80 d;
};
static_assert(AssertSize<FloatExtractor<f80>, sizeof(f80)>());
#endif

using F64ComponentType = unsigned long long;
...
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
    F64ComponentType sign : 1;
    F64ComponentType exponent : 11;
    F64ComponentType mantissa : 52;
#else
    F64ComponentType mantissa : 52;
    F64ComponentType exponent : 11;
    F64ComponentType sign : 1;
...
union FloatExtractor<f64> {
    ...
    static constexpr F64ComponentType mantissa_max = (1ull << 52) - 1;
    static constexpr int exponent_bias = 1023;
    static constexpr int exponent_bits = 11;
    static constexpr unsigned exponent_max = 2047;
    [[gnu::packed]] F64Fields fields;
    f64 d;
};
static_assert(AssertSize<FloatExtractor<f64>, sizeof(f64)>());

using F32ComponentType = unsigned;

struct F32Fields {
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
    F32ComponentType sign : 1;
    F32ComponentType exponent : 8;
    F32ComponentType mantissa : 23;
#else
    F32ComponentType mantissa : 23;
    F32ComponentType exponent : 8;
    F32ComponentType sign : 1;
...
union FloatExtractor<f32> {
    ...
    static constexpr F32ComponentType mantissa_max = (1 << 23) - 1;
    static constexpr int exponent_bias = 127;
    static constexpr int exponent_bits = 8;
    static constexpr F32ComponentType exponent_max = 255;
    [[gnu::packed]] F32Fields fields;
    f32 d;
};
